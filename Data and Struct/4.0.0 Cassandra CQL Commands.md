...
```
cql> CREATE TABLE views (
		name varchar,
		counts counter,
		PRIMARY KEY (name)
	 );
	 
cql> UPDATE views SET counts = counts + 2 WHERE name = 'Aario';
```
...
cql> CREATE TABLE test.set_frozen_list (
		id uuid PRIMARY KEY,
		tegument set<frozen<list<int>>>
	 );	 
cql> INSERT INTO test.set_frozen_list (id, tegument) VALUES (
		79998560-c144-11e6-9ffb-819d3fcf064a,
		{[100], [200]}
	 );
	
cql> CREATE TABLE test.frozen_list (
		id uuid PRIMARY KEY,
		tegument frozen<list<int>>
	 );	 
cql> INSERT INTO test.frozen_list (id, tegument) VALUES (
		79998560-c144-11e6-9ffb-819d3fcf064a,
		[200]
	 );
...
cql> CREATE TYPE IF NOT EXISTS test.fullname (
		first_name text,
		last_name text
	 );
cql> CREATE TABLE test.users (
...
```
cql> CREATE KEYSPACE IF NOT EXISTS test2 WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 } AND durable_writes = true;
cql> use test
