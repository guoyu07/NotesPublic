...
Element.extend({
	...
	*/
	...
	Returns:
		The element.
	*/
/*redefine by Lixinguang */
	empty: function(){
		Garbage.destroy(this.getElementsByTagName('*'));
		return this.setHTML('');
	}
...
var Garbage = {
   ...
   /* trash: function(elements){
	...
	destroy:function(elements){
		...
		try{
			var item=elements[i];
			if (window.ie &&'tagName' in item&&(/object/i).test(item.tagName)){
				for (var p in item){
					if ('function'==$type(item[p])) item[p] = function(){};
				}
				if(item.parentNode){
				  item.parentNode.removeChild(item);
				}
			}
		 if ('removeEvents' in item) item.removeEvents();
		 }catch(e){continue;}
	  }
	 // alert('stop');
	}
...
window.addListener('unload', function(){
	...
	Garbage.destroy(elements);
	if (window.ie) CollectGarbage();
});
