...
function drawSample() {
	...
	for (i=0;i<4;i++) {
		...
		if (used) {
			...
			buff += '<img class="coltbl-image" src="images/uarr.gif" alt="" onclick="shiftVar('+i+')">';
...
function drawVar(colNr,varNr) {
	...
	for (var i=20; i>=0; i--) {
		...
		for (var j=20; j>=0; j--) {
			...
			if (c.S[0]==S && c.V[0]==V) buff += 'background: red url(\'images/ring-w.gif\') center no-repeat; ';
...
function selectScheme(name) {
	usedScheme = name;
	for (var i=0; i<5; i++) {
		objGet('previmg-'+schemeNames[i]).src = 'images/prev_' + schemeNames[i] + ((name==schemeNames[i])?'-on':'') + '.gif';
		}
...
function searchRGB() {
	var hex = prompt('请输入6位数的RGB颜色代码(不带#) 如：FF99CC',RGBinput);
	if (!hex) return;
	RGBinput = hex;
	if (!hex.match(/^[0-9a-fA-F]{6}$/)) {
		alert('错误的RGB颜色代码，请重新输入！');
		return;
		}
