...
if (!empty($_POST['submit_post']))
{
	...
	if ($secresult != $seccode)
	{
		$error_message = "您所输入的验证码错误";
	}
	elseif (!$username || $username != addslashes($username))
	{
	...
	else
	{
		...
		{
			$logger_uid = $db->insert_id();
			$logger_user = $username;
			$logger_pw = md5($password);
			dsetcookie('sess_auth', authcode("{$logger_uid}\t{$logger_user}\t{$logger_pw}", 'ENCODE'), 86400, 1, true);
			updatesession();
			$url_forward = miniurl();
