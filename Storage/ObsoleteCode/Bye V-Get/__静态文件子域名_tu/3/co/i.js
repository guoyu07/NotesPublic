...
(function(){A('n','_self');
if(L($5.pathname)<2/*仅用于本地*/||/^\/[a-z]+\.wuliu\.v-get\.com\/?$/.test($5.pathname)/*仅用于本地*/){
...
if($k('view')&&('/logistics/news.html'==$5.pathname/*仅用于本地*/||/^\/[a-z]+\.wuliu\.v-get\.com\/logistics\/news\.html$/.test($5.pathname)/*仅用于本地*/)){
J('http://localhost/www.v-get.com/wuliu/i/static/js/bbcode.js',function(){
...
/* 第一个已经用 replace替换掉了 \n ，所以下面都可以用$r了 */
/* 不需要bbcode.js里面的 clearcode里，因为那个是对数据库操作的，所以数据库已经没有这些了*/

//用\\\[b\\\] 是避免如果用 \[b\] 会出现 [br]也匹配
...
/\[url\]\s*((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|thunder|qqdl|synacast){1}:\/\/|www\.)([^\[\"']+?)\s*\[\/url\]/ig
...
/* 百度分享 */
(function (b){
H(b,'<span class="bds_more"></span><a class="bds_tsina"></a><a class="bds_qzone"></a><a class="bds_tqq"></a><a class="bds_renren"></a><a class="bds_mshare"></a><a class="shareCount"></a>');
C(b,'ff mg bdshare_t bds_tools get-codes-bdshare');
