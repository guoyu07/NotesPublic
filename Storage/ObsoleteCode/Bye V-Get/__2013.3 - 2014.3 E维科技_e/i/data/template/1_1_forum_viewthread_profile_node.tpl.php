...
EOF;
 if($post['groupicon']) { 
$return .= <<<EOF
{$s}<a href="http://e.v-get.com/i/home.php?mod=spacecp&ac=usergroup&gid={$post['groupid']}" target="_blank">{$post['groupicon']}</a>{$e}
EOF;
 } 
$return .= <<<EOF
...
?><?php
$return = <<<EOF
{$s}<a href="http://e.v-get.com/i/home.php?mod=spacecp&ac=usergroup&gid={$post['groupid']}" target="_blank">{$post['authortitle']}</a>{$e}
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
{$s}<a href="http://e.v-get.com/i/home.php?mod=medal">{$return}</a>{$e}
EOF;
?><?php 
return $return;
}
