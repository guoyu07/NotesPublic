...
echo '<?xml version="1.0" encoding="utf-8"?><rss version="2.0"><channel>',
	(count($fidarray)>1?'<title>E维站长论坛</title><link>http://e.v-get.com/i/</link><description>E维站长论坛，沟通网站建设者、网络营销者与企业的平台，全力为站长、技术人员和企业提供PHP/JSP/ASP.NET等技术交流以及站长交易中介服务、注册机、注册码、站长工具、建站源码软件下载等服务。</description>':"<title>$forumname _E维科技_V-Get!</title><link>http://e.v-get.com/i/topic-$rssfid-1.html</link><description>$forumname 最新 $num 条帖子</description>"
	),
	'<lastBuildDate>',gmdate('r',TIMESTAMP),'</lastBuildDate><ttl>',$ttl,'</ttl><image><url>http://tp.v-get.com/j/i/e125x33.png</url><title>E维科技，专注网络营销！</title><link>http://e.v-get.com/</link></image>';
	...
	if($alldata) {
		foreach($alldata as $thread) {
			if(TIMESTAMP - $thread['lastupdate']>$ttl*60) {
				...
				echo '<item><title>',$thread['subject'],'</title><link>http://e.v-get.com/i/tip-',$thread[tid],'-1.html</link><description><![CDATA[',dhtmlspecialchars($thread['description']),']]></description><category>',dhtmlspecialchars($thread['forum']),'</category><author>http://e.v-get.com/i/',dhtmlspecialchars($thread['author']),'</author>',($attachfile?'<enclosure url="'.$filename.'" length="'.$attachsize.'" type="image/jpeg" />':''),'<pubDate>',gmdate('r',$thread['dateline']),'</pubDate></item>';
			}
		}
	} else {
		updatersscache($num);
	...
	foreach($_G['cache']['forums'] as $fid => $forum) {
		if($forum['type'] != 'group') {
			...
			foreach(C::t('forum_thread')->fetch_all_by_fid_displayorder($fid,0,null,null,0,$num,'tid') as $thread) {
				$thread['author'] = $thread['author'] != ''?addslashes($thread['author']):'E维科技';
