		...
		foreach(C::t('forum_thread')->fetch_all_by_fid_lastpost($fid, $time, TIMESTAMP) as $thread) {
			...
			if($forum_field['threadtypes']['prefix']) {
				if($forum_field['threadtypes']['prefix'] == 1) {
					$thread['threadtype'] = $forum_field['threadtypes']['types'][$thread['typeid']] ? '<em>[<a href="http://e.v-get.com/i/topic-'.$fid.'-1.html?filter=typeid&typeid='.$thread['typeid'].'">'.$forum_field['threadtypes']['types'][$thread['typeid']].'</a>]</em> ' : '' ;
				} elseif($forum_field['threadtypes']['prefix'] == 2) {
					$thread['threadtype'] = $forum_field['threadtypes']['icons'][$thread['typeid']] ? '<em><a href="http://e.v-get.com/i/topic-'.$fid.'-1.html?filter=typeid&typeid='.$thread['typeid'].'"><img src="'.$forum_field['threadtypes']['icons'][$thread['typeid']].'"/></a></em> ' : '' ;
				}
			}
			if($forum_field['threadsorts']['prefix']) {
				$thread['threadsort'] = $forum_field['threadsorts']['types'][$thread['sortid']] ? '<em>[<a http://e.v-get.com/i/topic-'.$fid.'-1.html?filter=sortid&typeid='.$thread['sortid'].'">'.$forum_field['threadsorts']['types'][$thread['sortid']].'</a>]</em>' : '' ;
			}
			if($thread['highlight']) {
			...
			if(in_array('forum_viewthread', $_G['setting']['rewritestatus'])) {
				...
				$thread['threadurl'] = '<a href="http://e.v-get.com/i/tip-'.$thread['tid'].'-1.html"'.$thread['highlight'].$target.'class="s xst z">'.$thread['subject'].'</a>';
			}
