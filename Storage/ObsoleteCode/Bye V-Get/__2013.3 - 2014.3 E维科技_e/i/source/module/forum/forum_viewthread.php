...
if($_GET['from'] == 'portal') {
	...
	$navigation=' <em>&rsaquo;</em> <a href="http://e.v-get.com/i/">'.$_G['setting']['navs'][3]['navname'].'</a> '.$nav['nav'];
	...
	if($_G['forum']['type'] == 'sub') {
		...
		$t_link=$_G['cache']['forums'][$fup]['type'] == 'group' ? 'http://e.v-get.com/i/bbs-'.$fup.'.html' : 'http://e.v-get.com/i/topic-'.$fup.'-1.html';
	...
	$t_link='http://e.v-get.com/i/topic-'.$_G['fid'].'-1.html'.($_GET['extra'] && !IS_ROBOT ? '?'.$_GET['extra'] : '');
	...
	if($archiveid) {
		...
		$navigation .= ' <em>&rsaquo;</em> <a href="http://e.v-get.com/i/topic-'.$_G['fid'].'-1.html?archiveid='.$archiveid.'">'.$t_name.'</a>';
	}
...
function viewthread_baseinfo($post, $extra) {
	...
	if(substr($key, 0, 10) == 'extcredits') {
		...
		$v=$type ? lang('space', 'viewthread_userinfo_sellercredit') : '<a href="http://e.v-get.com/i/home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#buyercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['buyercredit']).'.gif" /></a>';
	} elseif($key == 'eccredit_buyer') {
		$v=$type ? lang('space', 'viewthread_userinfo_buyercredit') : '<a href="http://e.v-get.com/i/home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#sellercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['sellercredit']).'.gif" /></a>';
	} else {
...
function getLinkByKey($key, $post, $returnarray=0) {
	...
	if(!$returnarray) {if($v['link']) {$v='<a href="http://e.v-get.com/i/'.$post['username'].'" target="_blank" class="xi2">'.$v['value'].'</a>';}else {$v=$v['value'];}}
	return $v;
}
