...
$Qq=mysql_query('SELECT h,he,k,r,q,s,p,lt,x,y,z,t,adt FROM co WHERE e="'.$E.'" LIMIT 1');$Qr=mysql_num_rows($Qq);if($Qr==0){header('Location: http://wuliu.v-get.com/');exit();}
...
<div class="o mh"></div>
					...
					(function(){
						var m=new BMap.Map("map"),p=new BMap.Point(<?php echo $Qa['p'];?>),k=new BMap.Marker(p),w; 
						/* 坐标 */
						m.centerAndZoom(p,13);/* 缩放比例 */
						...
						m.addOverlay(k);
						<?php echo 'w=new BMap.InfoWindow(',"'",'<div class="map"><img src="http://localhost/tp.v-get.com/j/3/co/70/',$E,'.gif" /><ul>',$Qa['r'],'<li>网　址：',$E,'.wuliu.v-get.com</li></ul></div>',"'",',{/*width:250,height:100,*/title:"',$H,'"});';?>
						
						
						k.openInfoWindow(w);
						k.addEventListener($7,function(){this.openInfoWindow(w);});
