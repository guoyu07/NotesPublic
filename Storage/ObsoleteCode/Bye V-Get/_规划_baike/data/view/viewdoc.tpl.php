  ...
  <div class="content_1 wordcut">
    ...
    <fieldset id="catalog">
      ...
      <ul id="hidesection">
        ...
        style="display:none"
        <?php } ?>
        >&#8226; <a href="wiki/<?php echo rawurlencode($doc['rawtitle']);?>#<?php echo $sec['key']?>"><?php echo $sec['value']?></a>
        </li>
        <?php }?>
      </ul>
    ...
    <?php } ?>
    <h3><span class="texts"><?php echo $section['value']?></span><a name="<?php echo $key?>" href="index.php?doc-editsection-<?php echo $doc['did']?>-<?php echo $key?>" >编辑本段</a><a href="wiki/<?php echo rawurlencode($doc['rawtitle']);?>#section">回目录</a></h3>
    <?php } else { ?>
...
<div class="bor_b-ccc m-t10 notes">
  ...
  <p class="r">
    ...
    上一篇<a href="wiki/<?php echo rawurlencode($neighbor['predoc']['rawtitle']);?>"  class="m-lr8"><?php echo $neighbor['predoc']['title']?></a>
    <?php } ?>
    <?php if($neighbor['nextdoc']) { ?>
    下一篇<a href="wiki/<?php echo rawurlencode($neighbor['nextdoc']['rawtitle']);?>"  class="m-lr8"><?php echo $neighbor['nextdoc']['title']?></a>
    <?php } ?>
  </p>
</div>
	...
	$.post(
		...
		function(xml){
			...
			if(message=='1'){
				...
				$("#updatetitlenotice").html('相同的同义词名称已存在');
			}else{
