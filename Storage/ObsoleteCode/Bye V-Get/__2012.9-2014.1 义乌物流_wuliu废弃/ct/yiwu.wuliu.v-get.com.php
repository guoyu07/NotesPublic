...
<?php
/*<!--不采用c1.php/c2.php/c3.php,因为这样避免了对局部广告或信息改变，需要改变多次的情况-->
   文件名不用简称命名，全部使用关键词，因为搜索引擎也会搜索链接地址的，所以用关键词地址更利于SEO
   如果使用伪静态伪目录，最好用关键词传递参数，比如wuliu.php?ct=yiwu&id=100 用 wuliu/tuoyun-yiwu/huoche-100 这样自己添加关键词地址更利于SEO优化
   RewriteRule ^wuliu/.*-(.*)/.*-([0-9]+)\.html$ /wuliu.php?ct=$1&id=$2*/


/*$a= a=类型id   $t = 类型名称  $b= b=所在地代码  $n= n=native name所在地 (为了伪静态，n不存在，而且出现n和b矛盾的时候就有问题)    $k= k=目的地代码、关键词  |||||||||  $c= c=附近id，最详细的
  $sp =page 页码*/
$QC=mysql_connect('localhost','root','qwdw114');mysql_select_db('v3',$QC);mysql_query('set names utf8');  /* c.php要用 utf-8无bom编码，不然会出现bom问题 */
...
$Qo='o DESC';
/*
if(!empty($_GET['vips'])){$Qo='v DESC,o DESC';$key.='&vips=1';}
if(!empty($_GET['xs'])){$Qo='x DESC,o DESC';$key.='&xs=1';}
if(!empty($_GET['uvs'])){$Qo='u DESC,o DESC';$key.='&uvs=1';}
if(!empty($_GET['rates'])){$Qo='ks DESC,o DESC';$key.='&rates=1';}
if(!empty($_GET['prices'])){$Qo='kp DESC,o DESC';$key.='&prices=1';}
*/
$jsk='';
if(empty($_GET['sk'])){
...
$Qk=$_GET['sk'];//preg_replace不支持中文，所以要尾部加/u ,才能支持utf-8
...
//$Qk=mb_substr($Qk,0,15,'utf-8');  //截取15个字符以内
...
<ul>
  <li><a href="http://localhost/yiwu.wuliu.v-get.com/tuoyun/s?sk=外贸进仓"><span class="f1">外贸进仓</span></a>：<a href="http://localhost/yiwu.wuliu.v-get.com/tuoyun/s?sk=外贸进仓">义乌到宁波外贸进仓便宜到家</a></li>
...
<!--<select name="sc"><option value="0"></option><option value="1">稠城街道</option><option value="2">江东街道</option><option value="3">后宅街道</option><option value="4">稠江街道</option><option value="5">廿三里</option><option value="6">北苑城西</option><option value="7">上溪义亭</option><option value="8">佛堂赤岸</option><option value="9">苏溪大陈</option></select>-->
...
echo '<div class="f cc"><div class="cct">|&nbsp;&nbsp;义乌',$a,'<span>约',$Qt,'个</span></div><div id="c">';
	 ...
	 echo '<div class="cv">
		 <h2><a href="http://localhost/wuliu.v-get.com/',$I,'" class="h2c">',$h,'</a>';
		 if($bM){echo '<a href="http://localhost/',$M,'.wuliu.v-get.com/">[官网]</a>';}
		 echo '<a v="',$v,'"></a><a o="',$Qa['o'],'"></a><a href="#" class="h2r pr" b="',$I,'"></a></h2><div class="o p cl"><a href="http://localhost/';
		 if($bM){echo $M,'.wuliu.v-get.com"><img src="http://localhost/www.v-getimg.com/i1/s3/50/',$M,'.gif" alt="',$h,'';}
		 else {echo 'wuliu.v-get.com/',$I,'"><img src="http://localhost/www.v-getimg.com/i1/s3/50/',$bP,'.gif" alt="',$h,'';}
		 echo '"></a>
		 </div>
		 <div class="p cr"><p>',$Qa['d'],'</p></div>
	 <div class="o cb"><div class="p">wuliu.v-get.com/',$I,'&nbsp;&nbsp;',$Qa['t'],'</div><div q="';
	  if($bM){echo 'W',$M,'.wuliu.v-get.com^';}
	 echo $Qa['q'],'^Jlocalhost/yiwu.wuliu.v-get.com/',$sa,'-price/';
	 if($bM){echo '?sco=',$M;}
	 echo '^Dwuliu.v-get.com/',$I,'map" class="q"></div></div>
...
$Qpq=($Qpp==1)?'':'-'.$Qpp;
switch ($sp)
{ case 1:echo '<div id="p"><a class="po">首页</a><a class="po">前一页&lt;</a>第<span class="pc">1</span>页<a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'-2/',$key,'" target="_self">&gt;下一页</a><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'-',$Qpz,'/',$key,'" target="_self">共',$Qpz,'页</a></div>';break;
 case $Qpz:echo '<div id="p"><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'/',$key,'" target="_self">首页</a><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,$Qpq,'/',$key,'" target="_self">前一页&lt;</a>第<span class="pc">',$sp,'</span>页<a class="po">&gt;下一页</a><a class="po">第',$sp,'页</a></div>';break;
  default:echo '<div id="p"><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'/',$key,'" target="_self">首页</a><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,$Qpq,'/',$key,'" target="_self">前一页&lt;</a>第<span class="pc">'.$sp.'</span>页<a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'-',$Qpn,'/',$key,'" target="_self">&gt;下一页</a><a href="http://localhost/yiwu.wuliu.v-get.com/',$sa,'-',$Qpz,'/',$key,'" target="_self">第',$Qpz,'页</a></div>';break;
}
}
echo '</div>';
...
<div class="r2 rw"><h4>|<a href="http://localhost/yiwu.wuliu.v-get.com/news/">义乌物流</a></h4><ul><?php
...
<div class="mg" a="200x200"></div>

<div class="r4 rw mg">
<h4>|<a href="http://localhost/yiwuwuliu.baike.v-get.com/logistics/index_1.html">物流百科</a></h4>
...
J('http://localhost/www.v-getimg.com/i0/s3/a.js');



//这句话如果放到J(function(){})里面就会出问题了！！！背景触发无法变色，所以需要注意

function cK(s,n){F($p(s,","),function(k){if(T(k)){H($("c"),$r(H($("c")),RegExp("(>[^<]{0,})("+k+")([^>]{0,}<)","ig"),"$1<span class=\"f0\">$2</span>$3"))}});C($("n^a")[n],"no")}cK(<?php echo '"',$jsk,'",',$A+2;?>);
