...
/* i||0 如果 i （不存在、为0、NaN），那么返回0  ，而如果i（负数、正数），返回i；不熟悉 || 就看 http://e.v-get.com/tech/20131111/1117231135.html*/
function $i(i){return parseInt(i||0)}
...
// L(0)=0 ，L('0')=1 ==>  s（不存在、0、false、NaN、NULL）不执行s.length，直接 false||0 ，返回0，不熟悉与或阅读http://e.v-get.com/tech/20131111/1117231135.html
// 自然数（包括0、负数、整数）长度都为0
...
// -1 是 true
function T(s,t){return t?($l(typeof(s))==['','undefined','boolean','number','string','object','function'][t]):((s||s===0)?I:O)}
...
// v=0 || v="" 都可以执行赋值
function $g(o,a,v){return (T(v)||v=="")?o.setAttribute(a,v):o.getAttribute(a)}


//重右往左选取字符是，负数，如果单独一个$s(arr) 就是返回热荐关键词，随机输出一个
function $s(x,f,l){return T(f)?x.substr(f,l):x[$i(Math.random()*L(x))]}
...
function $b(o,x,y){var p="px";o.style.backgroundPositionX=x+p;o.style.backgroundPositionY=y+p;o.style.backgroundPosition=x+"px "+y+p}
/* 将被替换成$b() */
function SP(o,x,y){return $b(o,x,y)}



/* $C(tagName,id=e/class=love,html,$4.body)  新建元素，为了预防 html=0 ，所以暂时使用 T(h) 而不是 h*/
function $C(e,g,h,p){var o=$4.createElement(e||'div'),a;if(T(g)){a=$p(g,'=');$g(o,a[0],a[1]);}T(h)&&H(o,h);p=T(p,5)?p:$4.body;p.appendChild(o);e=="script"&&(o.type="text/javascript");return o}

function Z(h){if($e()!='i6'){$C(O,'id=z',h);A();E($('^a.zo')[0],$7,function(){D($('z'))});}}
...
function $k(c,d,l){var r='';c+="=";if($x(l||$5.search,'?')<0)return O;F($p(l?($p(l,'?')[1]):($s($5.search,1)),'&'),function(i){if(c==$s(i,0,L(c)))r=$s(i,L(c));});return T(r)&&(d?r:decodeURI(r))}
function $K(o,s){F($p(s,","),function(k){if(k){H(o,$r(H(o),RegExp("(>[^<]{0,})("+k+")([^>]{0,}<)","ig"),"$1<span class=\"f0\">$2</span>$3"))}})}

// d=cd  $D(a外面的id,显示区的tagName,是否有class ，默认  id=?da  cda     class=?d 或者 id=?d   ?do 
function $D(d,r){var s=d+'a',a=$(s+'^a'),g,c;F(a,function(o,i){if(i>0)D(r[i]);E(o,$8,function(){g=this;F(a,function(v,j){c=C(v);D(r[j]);C(v,$r(c,RegExp('\s?'+s,'g'),''));if(v==g){D(r[j],1);C(v,$r(c+' ',/\s{2,}/g,' ')+s)}})})})}

// 注意 C(obj,"") 将class设置为空的情况
...
//要注意 ，s 作为InnerHTML为了图方便，会用number，注意H(obj,100) H(obj,"") 的现象
...
*/
function $(i,o){i=$r(i,/^\./,'^*.');var s=$p(i,'^'),a=s[0],g=function(x){return $4.getElementById(x)},r=a&&g(a);
// 
// !a === L(a)==0，如果 i中不包含id（那么必然有tag），或者包含id，并且已经找到
// && 优先级大于 || ，请见http://e.v-get.com/tech/20131111/1117231135.html
// 下面即使 a为空，也可以使用 g(a)，因为 || 第一个!a已经过滤了，不为空
if((!a||g(a))&&s[1]){var c=$p(s[1],'.'),d=c[0],e=c[1],v=[];o=r||o||$4;r=o.getElementsByTagName(d);if(e){F(r,function(o){F($p(C(o),' '),function(c){if(c==e){v=v.concat(o)}})});r=v}}return r}
...
*/
function A(o,t,y,z){var k='_blank',h,g,l,j="javascript:",e;
F($((o||'')+"^a"),function(i){
h=i.href;g=i.target;l='s.v-get.com/l?l=';
//这里不能用if(L(h)<4)，因为即使是 #都会带网址
// 
if($s(h,-1)=="#")i.href=j+"void(0)";
else if(j!=$s(h,0,11)&&$p(h,"#")[0]!=$4.URL&&$g(i,k)!='_'&&(!g||g==k||t=='_')){i.target=(L(t)>1)?t:k;$g(i,k,t);/* L(t)>0 && t!='_'  综合就是 L(t)>1，有时候iframe会是两位命名的*/
if($x(h,l)>0){h=$r(h,/%\w{2}/g,function(a){return String.fromCharCode($i('0x'+$s(a,1)))});i.href=$r(h,l,'')}

if(!i.title){
// 显示a的 title,取消 <a><img>dfasf&lt;</img></a>的 <img> 和  &lt;等情况
e=H(i);
F([['<[^>]+>',''],['&#34;','"'],['&#39;',"'"],['&lt;','<'],['&gt;','>']],function(u){e=$r(e,RegExp(u[0],"g"),u[1])});
i.title=e||i.href;}
...
function K(k,v,d){var x=$4.cookie,c,e,t=new Date();if(v){t.setDate(t.getDate()+d||9);$4.cookie=k+"="+escape(v)+";expires="+t.toGMTString()}else{if(T(x)){c=$x(x,k+"=");if(c!=-1){c=c+L(k)+1;e=$x(x,";",c);if(e==-1)e=L(x);return unescape(x.substring(c,e))}}return ""}
}
//A(l,v,o,f)   ajax会出现跨域问题，此时a.status=0;返回0.这时必须把  l在本网站下，比如 yiwu.wuliu.v-get.com    function 不能用 N(f)判断，function/object直接用 if(h)
function $A(l,v,o,f){var a=null,s,h;try{a=new XMLHttpRequest()}catch(e){try{a=new ActiveXObject("Msxml2.XMLHTTP")}catch(e){a=new ActiveXObject("Microsoft.XMLHTTP")}}a.onreadystatechange=function(){s=a.readyState;if(s==4||s=="complete"){h=a.responseText;o&&H(o,h);f&&f(h)}};a.open("POST",l,I);a.setRequestHeader("Content-Type","application/x-www-form-urlencoded");a.send("v="+v)}
...
// c>0 返回c，否则返回$i(g?o.offsetWidth:o.offsetHeight)
return $i(c)||$i(g?o.offsetWidth:o.offsetHeight)
}
...
// z=(p>0&&p<3)?I:O  用逻辑与表示http://e.v-get.com/tech/20131111/1117231135.html
var c=$m(a,g),l,w,u=$('^ul',o)[0],k=H(u),b,r,q=0,z=p>0&&p<3,h;
...
//while(c%v!=0){v++}  //必须要这样，不然如果v不能被c整除，就会出现滑动次数过多
 ...
 //z?(o[d]<1):(o[d]>=w/2)  可以用逻辑与  z&&(o[d]<1)||(o[d]>=w/2)  表示，但是用逻辑与字符更多，所以保持三元
 r=function(){if(o[d]%l>0){x(z);if(z?(o[d]<1):(o[d]>=w/2))o[d]=j}else{$o(y);$o(b,t)}};
...
//t*i+z
d=d||'n';
var q=d+'a^',p='px',n=$('^div.'+d),o=$(q+'a'),x=L(o),c=$(q+'a')[0],g=$i($r(S(c,'marginLeft'),p,'')),e=$e(),v=(e=='i6'||e=='i7'),u=(v||e=='i9');
...
function k(i){var m=$m($(q+'i')[0],1),r=i==0?0:-m,g=$i($r(S($(q+'a')[0],'marginLeft'),p,''));F($(q+'a'),function(a,k){r+=k<i?($m(a,1)+m):0});return i<Math.ceil(x/2)?(r+(i==0?0:(g*2*i+g*2))+g-1):((t?t:60)*i+(z||6))}
//这里m必须在内部var，不然出错
...
*/
r=r||96;v=Math.PI/(v||324);d=d||300;u=u||200;
 ...
 */
 ...
 var s=$C("script");
        //IE浏览器  //非IE
        s.readyState?(s.onreadystatechange=function(){if(s.readyState=="loaded"||s.readyState=="complete"){s.onreadystatechange=null;f&&f(s)}}):(s.onload=function(){f&&f(s)});
         s.src=l;
      }
...
alert(s1+" "+s2+" "+s3)//结果为1985 10 15 */
  ...
  $4.ready=function (f) {
    ...
    else w&&(t=$o(function(){if(/^(loaded|complete)$/.test($4.readyState))$o(t,I);r()},0,I));
  };
})();
