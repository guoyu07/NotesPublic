...
if($_GET['from'] == 'portal') {
	...
	$navigation=' <em>&rsaquo;</em> <a href="http://wuliu.v-get.com/i/">'.$_G['setting']['navs'][3]['navname'].'</a> '.$nav['nav'];
	...
	if($_G['forum']['type'] == 'sub') {
		...
		$t_link=$_G['cache']['forums'][$fup]['type'] == 'group' ? 'http://wuliu.v-get.com/i/bbs-'.$fup.'.html' : 'http://wuliu.v-get.com/i/topic-'.$fup.'-1.html';
	...
	$t_link='http://wuliu.v-get.com/i/topic-'.$_G['fid'].'-1.html'.($_GET['extra'] && !IS_ROBOT ? '?'.$_GET['extra'] : '');
	...
	if($archiveid) {
		...
		$navigation .= ' <em>&rsaquo;</em> <a href="http://wuliu.v-get.com/i/topic-'.$_G['fid'].'-1.html?archiveid='.$archiveid.'">'.$t_name.'</a>';
	}
...
function viewthread_baseinfo($post, $extra) {
	...
	if(substr($key, 0, 10) == 'extcredits') {
		...
		$v=$type ? lang('space', 'viewthread_userinfo_sellercredit') : '<a href="http://wuliu.v-get.com/i/home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#buyercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['buyercredit']).'.gif" /></a>';
	} elseif($key == 'eccredit_buyer') {
		$v=$type ? lang('space', 'viewthread_userinfo_buyercredit') : '<a href="http://wuliu.v-get.com/i/home.php?mod=space&uid='.$post['uid'].'&do=trade&view=eccredit#sellercredit" target="_blank" class="vm"><img src="'.STATICURL.'image/traderank/seller/'.countlevel($post['sellercredit']).'.gif" /></a>';
	} else {
...
function getLinkByKey($key, $post, $returnarray=0) {
	switch($key) {
		...
		case 'posts': $v=array('link' =>$post['username'], 'value' => $post['posts']);break;
		case 'threads': $v=array('link' =>$post['username'], 'value' => $post['threads']);break;
		case 'digestposts': $v=array('link' =>$post['username'], 'value' => $post['digestposts']);break;
		case 'feeds': $v=array('link' => $post['username'], 'value' => $post['feeds']);break;
		case 'doings': $v=array('link' =>$post['username'], 'value' => $post['doings']);break;
		case 'blogs': $v=array('link' =>$post['username'], 'value' => $post['blogs']);break;
		case 'albums': $v=array('link' =>$post['username'], 'value' => $post['albums']);break;
		case 'sharings': $v=array('link' =>$post['username'], 'value' => $post['sharings']);break;
		case 'friends': $v=array('link' =>$post['username'], 'value' => $post['friends']);break;
		case 'follower': $v=array('link' =>$post['username'], 'value' => $post['follower']);break;
		case 'following': $v=array('link' =>$post['username'], 'value' => $post['following']);break;
		case 'credits': $v=array('link' =>$post['username'].'?do=profile', 'value' => $post['credits']);break;
	...
	if(!$returnarray) {if($v['link']) {$v='<a href="http://wuliu.v-get.com/i/'.$post['username'].'" target="_blank" class="xi2">'.$v['value'].'</a>';}else {$v=$v['value'];}}
	return $v;
}
