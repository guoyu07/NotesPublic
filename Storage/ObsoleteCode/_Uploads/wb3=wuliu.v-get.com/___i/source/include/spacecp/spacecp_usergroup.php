...
if(in_array($do, array('buy', 'exit'))) {
	...
	foreach($_G['cache']['usergroups'] as $gid => $group) {
		...
		$g='<a href="http://wuliu.v-get.com/i/home.php?mod=spacecp&ac=usergroup&gid='.$gid.'"'.(!empty($_GET['gid'])&&$_GET['gid']==$gid?' class="xi1"' : '').'>'.$group['grouptitle'].'</a>';
	...
	$usergroups['my']='<a href="http://wuliu.v-get.com/i/home.php?mod=spacecp&ac=usergroup">'.$maingroup['grouptitle'].'</a>'.$usergroups['my'];
	if($activegs==array()) {
		$activegs['my']=' a';
	}
