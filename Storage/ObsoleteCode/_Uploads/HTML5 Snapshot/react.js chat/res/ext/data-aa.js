/**
 * *****************************************************************************
 * 刚看到html5,发现HTML5很多东西和我这个设计思路重叠度太高了~~~但是设计这个是
 * 2014 年，当时我并没有看HTML5。当时的代码比较乱（https://github.com/lefwell/_ObsoleteCodes/blob/master/zy201211%20Dead%20JS%20function%20package/201405%20ys.js） 。
 * 当时开发这个其实是为了第二个功能，自动化管理后台。今天（2015-05-26）看到HTML5
 * 很多重叠的功能，我就懒得继续玩data-lef了。
 * *****************************************************************************
 *
 ...
 * @var data-lef-errshow class/id where to show the errmsg,
 *  default is lef.errmsgsClass:'.data-lef-errmsg' off is cancel
...
(function () {
    ...
    $.fn.extend(
        {
            ...
            'dataLef': function (s, flg) {
                ...
                if (a = $(this).data('lef-' + s)) {
                    if ($.inArray(s, ['errmsg', 'len', 'lenrange', 'temporary', 'regexp', 'type', 'calc']) > -1)
    ...
    f.prototype = {
        ...
        errmsgsClass: '.data-lef-errmsg',
        /**
         * @return string default tip shows in the errmsgsClass
         */
        defaulterrmsg: function (s) {
            return s + ' ERROR';
        },
        htmlTags: [
            ['—', '&#8212;'],
        ...
        showErr: function (o, s) {
            ...
            s = s || $(o).dataLef('errmsg') || lef.defaulterrmsg($(o).attr('placeholder') || $(o).attr('name'));
            if (errshow == 'off')
                return;
            if (!errshow) {
                if ($(o).parents('form').find(lef.errmsgsClass).length < 1)
                    $(o).parents('form').prepend('<p class="' + lef.errmsgsClass.replace(/[^\w\-]/g, '') + '">' + s + '</p>');
                errshow = $(o).parents('form').find(lef.errmsgsClass);
            }
        ...
        getLenrange: function (o) {
            ...
            if (range) {      // int|range
                ...
                var r = [];
                if (!e[2])
                    return [parseInt(e[1]), parseInt(e[1])];
        ...
        /**
         * It changes always, so it should be combined handle actions with check
         */
        ...
        beforeSubmit: function (o, e) {
            ...
            if (!$(o).is('form'))
                o = $(o).parents('form');
            $(o).find(lef.errmsgsClass).hide(180);
