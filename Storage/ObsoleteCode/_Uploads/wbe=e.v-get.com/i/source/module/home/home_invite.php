...
if($acceptconfirm) {
	...
	if($id) {
		...
		notification_add($uid, 'friend', 'invite_friend', array('actor' => '<a href="http://e.v-get.com/i/'.$_G['username'].'" target="_blank">'.$_G['username'].'</a>'), 1);
	}
	space_merge($space, 'field_home');
	if(!empty($space['privacy']['feed']['invite'])) {
		require_once libfile('function/feed');
		$tite_data = array('username' => '<a href="http://e.v-get.com/i/'.$_G['username'].'">'.$_G['username'].'</a>');
