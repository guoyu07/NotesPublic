...
if($_G['forum']['type'] == 'forum') {
	...
	if(empty($_GET['archiveid'])) {
		$navigation=' <em>&rsaquo;</em> <a href="http://e.v-get.com/i/bbs-'.$forum_up['fid'].'.html">'.$forum_up['name'].'</a><em>&rsaquo;</em> <a href="http://e.v-get.com/i/topic-'.$_G['forum']['fid'].'-1.html">'.$_G['forum']['name'].'</a>';
	} else {
		$navigation=' <em>&rsaquo;</em> '.'<a href="http://e.v-get.com/i/topic-'.$_G['fid'].'-1.html">'.$_G['forum']['name'].'</a> <em>&rsaquo;</em> '.$forumarchive[$_GET['archiveid']]['displayname'];
	}
	...
	if(empty($_GET['archiveid'])) {
		...
		$navigation=' <em>&rsaquo;</em> <a href="http://e.v-get.com/i/bbs-'.$forum_top['fid'].'.html">'.$forum_top['name'].'</a><em>&rsaquo;</em> <a href="http://e.v-get.com/i/topic-'.$forum_up['fid'].'-1.html">'.$forum_up['name'].'</a><em>&rsaquo;</em> '.$_G['forum']['name'];
	} else {
		$navigation=' <em>&rsaquo;</em> <a href="http://e.v-get.com/i/topic-'.$_G['forum']['fup'].'-1.html">'.$forum_up['name'].'</a> <em>&rsaquo;</em> '.'<a href="http://e.v-get.com/i/topic-'.$_G['fid'].'-1.html">'.$_G['forum']['name'].'</a> <em>&rsaquo;</em> '.$forumarchive[$_GET['archiveid']]['displayname'];
	}
	$seodata=array('forum' => $_G['forum']['name'], 'fup' => $forum_up['name'], 'fgroup' => $forum_top['name'], 'page' => intval($_GET['page']));
}

$rssauth=$_G['rssauth'];
$rsshead=$_G['setting']['rssstatus'] ? ('<link rel="alternate" type="application/rss+xml" title="'.$_G['setting']['bbname'].' - '.$navtitle.'" href="http://e.v-get.com/i/forum.php?mod=rss&fid='.$_G['fid'].'&amp;auth='.$rssauth."\" />\n") : '';
...
foreach($threadlist as $thread) {
	...
	if($thread['typeid'] && !empty($_G['forum']['threadtypes']['prefix']) && isset($_G['forum']['threadtypes']['types'][$thread['typeid']])) {
		if($_G['forum']['threadtypes']['prefix'] == 1) {
			$thread['typehtml']='<em>[<a href="http://e.v-get.com/i/topic-'.$_G['fid'].'-1.html?filter=typeid&amp;typeid='.$thread['typeid'].'">'.$_G['forum']['threadtypes']['types'][$thread['typeid']].'</a>]</em>';
	...
	$thread['sorthtml']=$thread['sortid'] && !empty($_G['forum']['threadsorts']['prefix']) && isset($_G['forum']['threadsorts']['types'][$thread['sortid']]) ?
		'<em>[<a href="http://e.v-get.com/topic-'.$_G['fid'].'-1.html?filter=sortid&amp;sortid='.$thread['sortid'].'">'.$_G['forum']['threadsorts']['types'][$thread['sortid']].'</a>]</em>' : '';
	$thread['multipage']='';
	...
	foreach(C::t('common_member_verify')->fetch_all($verifyuids) as $value) {
		foreach($_G['setting']['verify'] as $vid => $vsetting) {
			if($vsetting['available'] && $vsetting['showicon'] && $value['verify'.$vid] == 1) {
				...
				$verify[$value['uid']] .= "<a href=\"http://e.v-get.com/i/home.php?mod=spacecp&ac=profile&op=verify&vid=$vid\" target=\"_blank\">".(!empty($srcurl) ? '<img src="'.$srcurl.'" class="vm" alt="'.$vsetting['title'].'" title="'.$vsetting['title'].'" />' : $vsetting['title']).'</a>';
			}
		}
