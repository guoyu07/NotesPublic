var re, DISCUZCODE = [];DISCUZCODE['num'] = '-1';DISCUZCODE['html'] = [];EXTRAFUNC['bbcode2html'] = [];EXTRAFUNC['html2bbcode'] = [];

function addslashes(s) {return preg_replace(['\\\\', '\\\'', '\\\/', '\\\(', '\\\)', '\\\[', '\\\]', '\\\{', '\\\}', '\\\^', '\\\$', '\\\?', '\\\.', '\\\*', '\\\+', '\\\|'], ['\\\\', '\\\'', '\\/', '\\(', '\\)', '\\[', '\\]', '\\{', '\\}', '\\^', '\\$', '\\?', '\\.', '\\*', '\\+', '\\|'],s);}
...
if(!l.match(/.+\..+/))return '#';
if(!l.split('/')[0].match(RegExp(d.replace('.','\.')))){l=encodeURIComponent(l);l='http://s.v-get.com/l?l='+l;}
l=(l.substr(0,4)=='http')?l:('http://'+l);
return l
}
function VGT_DelAttachUrl(s){
return s.replace(/\[url[^\]]*\](\[attach[^\]]+\]\d+\[\/attach[^\]]*\])\[\/url\]/gi,'$1')
}

/* atag('href="http://e.v-get.com/i/ target="_blank"','http://e.v-get.com/i/') */
function atag(h,s){
	if(trim(s)=='')return '';
	var p=parsestyle(h,'',''),l=getoptionvalue('href',h);
	if(l.substr(0,11)=='javascript:')return trim(recursion('a',s,'atag'));
	/* discuz有对 &进行 &amp;处理 */
     l=vget_url(l.replace('^.+href="([^"]+)".+$','$1').replace(/&amp;/g,'&'));
	
	 s=trim(recursion('a',s,'atag'));
	return p['prepend']+(/^http\:\/\/[\w\.]+(\.v\-get|weigeti)\.com\/i\/$/.test(s)?s:('[url='+l+']'+s+'[/url]'))+p['append'];
}
