...
(function(){var i=0,e=!!(window.attachEvent&&!window.opera),w=/webkit\/(\d+)/i.test(navigator.userAgent)&&(RegExp.$1<525),a=[];var r=function(){for(i;i<a.length;i++)a[i]()},d=document,t;d.ready=function(f){if(!e&&!w&&d.addEventListener)return d.addEventListener('DOMContentLoaded',f,false);if(a.push(f)>1)return;if(e)(function(){try{d.documentElement.doScroll('left');r()}catch(err){setTimeout(arguments.callee,0)}})();else if(w)var t=setInterval(function(){if(/^(loaded|complete)$/.test(d.readyState))clearInterval(t);r()},0);};})();
document.ready(function(){var o=document.getElementsByTagName('a'),l='s.v-get.com/l?l=';for(var i=0;i<o.length;i++){if(o[i].href.indexOf(l)>0){
o[i].href=o[i].href.replace(/%\w{2}/g,function(a){return String.fromCharCode(parseInt('0x'+a.substr(1)))}).replace(l,'');}}});